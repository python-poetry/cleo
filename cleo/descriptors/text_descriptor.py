...
class TextDescriptor(Descriptor):
    ...
    def _describe_input_argument(self, argument, **options):
        ...
        if default is not None and (not isinstance(default, list) or len(default)):
            default = "<comment> [default: %s]</comment>" % self._format_default_value(
                default
            )
        else:
            default = ""
    ...
    def _describe_input_option(self, option, **options):
        ...
        if (
            ...
            and (not isinstance(default, list) or len(default))
        ):
            default = "<comment> [default: %s]</comment>" % self._format_default_value(
                default
            )
    ...
    def _describe_input_definition(self, definition, **options):
        ...
        if definition_arguments:
            ...
            for argument in definition_arguments:
                self._describe_input_argument(
                    argument, total_width=total_width, **options
                )
                self._write_text("\n")
    ...
    def _describe_command(self, command, **options):
        ...
        for usage in (
            [command.get_synopsis(True)] + command.get_aliases() + command.get_usages()
        ):
            self._write_text("\n")
    ...
    def _describe_application(self, application, **options):
        ...
        if raw_text:
            ...
            for command in description.get_commands().values():
                self._write_text(
                    "%-*s %s" % (width, command.get_name(), command.get_description()),
                    **options
                )
                self._write_text("\n")
        else:
            ...
            self._describe_input_definition(
                InputDefinition(application.get_definition().get_options()), **options
            )
            ...
            for namespace in description.get_namespaces().values():
                if (
                    ...
                    self._write_text("\n")
                    self._write_text(
                        " <comment>%s</comment>" % namespace["id"], **options
                    )
