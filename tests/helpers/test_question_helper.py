...
class QuestionHelperTest(CleoTestCase):
    def test_ask_choice(self):
        ...
        self.assertEqual(
            "Batman", question_helper.ask(self.get_input(), output, question)
        )

        output.get_stream().seek(0)
        self.assertRegex(
            decode(output.get_stream().read()), 'Input "John" is not a superhero!'
        )
    ...
    def test_ask(self):
        ...
        self.assertEqual(
            "2PM", dialog.ask(self.get_input(), self.get_output_stream(), question)
        )
        output = self.get_output_stream()
    ...
    def test_ask_hidden_response(self):
        ...
        self.assertEqual(
            "8AM", dialog.ask(self.get_input(), self.get_output_stream(), question)
        )
    ...
    def test_ask_confirmation(self):
        ...
        for d in data:
            ...
            self.assertEqual(
                d[1], dialog.ask(self.get_input(), self.get_output_stream(), question)
            )
    ...
    def test_ask_confirmation_with_custom_true_answer(self):
        ...
        question = ConfirmationQuestion(
            "Do you like French fries?", False, "(?i)^(j|y)"
        )
        self.assertTrue(
    ...
    def test_ask_and_validate(self):
        ...
        self.assertEqual(
            "white", dialog.ask(self.get_input(), self.get_output_stream(), question)
        )
        self.assertEqual(
            "black", dialog.ask(self.get_input(), self.get_output_stream(), question)
        )
    ...
    def test_select_choice_from_simple_choice(self):
        possible_choices = ["My environment 1", "My environment 2", "My environment 3"]
        ...
        for d in data:
            ...
            question = ChoiceQuestion(
                "Please select the environment to load", possible_choices
            )
            question.max_attempts = 1
