...
class ArgvInputTest(CleoTestCase):
    def test_init(self):
        ...
        self.assertEqual(["foo"], input_._tokens)
        self.assertEqual(["cli.py", "foo"], sys.argv)

    def test_parse_arguments(self):
        input_ = ArgvInput(["cli.py", "foo"])
        input_.bind(InputDefinition([InputArgument("name")]))
    ...
    def test_parse_options(self):
        for argv, options, expected_options, message in self.provide_options():
            ...
            self.assertEqual(expected_options, input_.get_options(), msg=message)
    ...
    def provide_options(self):
        return [
            ...
            (
                ["cli.py", "-fb"],
                [InputOption("foo", "f"), InputOption("bar", "b")],
                {"foo": True, "bar": True},
    ...
    def test_invalid_input(self):
        ...
        ) in self.provide_invalid_input():
            input_ = ArgvInput(argv)
            self.assertRaisesRegexp(
                Exception, expected_exception_message, input_.bind, definition
            )
    ...
    def provide_invalid_input(self):
        return [
            (
                ["cli.py", "--foo"],
                InputDefinition([InputOption("foo", "f", InputOption.VALUE_REQUIRED)]),
            ...
            (
                ["cli.py", "-ffoo"],
                InputDefinition([InputOption("foo", "f", InputOption.VALUE_NONE)]),
            ...
            (["cli.py", "foo", "bar"], InputDefinition(), "Too many arguments."),
            (
                ["cli.py", "--foo"],
                InputDefinition(),
                'The "--foo" option does not exist.',
            ),
            (["cli.py", "-f"], InputDefinition(), 'The "-f" option does not exist.'),
            (
                ["cli.py", "-1"],
                InputDefinition([InputArgument("number")]),
    ...
    def test_parse_list_argument(self):
        ...
        input_.bind(InputDefinition([InputArgument("name", InputArgument.IS_LIST)]))

        self.assertEqual({"name": ["foo", "bar", "baz", "bat"]}, input_.get_arguments())
    ...
    def test_parse_list_option(self):
        ...
        self.assertEqual({"name": ["foo", "bar", "baz"]}, input_.get_options())

        input_ = ArgvInput(
            ["cli.py", "--name", "foo", "--name", "bar", "--name", "baz"]
        )
        input_.bind(
            InputDefinition(
                [
        ...
        self.assertEqual({"name": ["foo", "bar", None]}, input_.get_options())

        input_ = ArgvInput(
            ["cli.py", "--name", "foo", "--name", "bar", "--name", "--another-option"]
        )
        input_.bind(
            InputDefinition(
                [
    ...
    def test_negative_number_after_double_dash(self):
        ...
        input_.bind(InputDefinition([InputArgument("number")]))
    ...
    def test_parse_single_dash_argument(self):
        ...
        input_.bind(InputDefinition([InputArgument("file")]))
