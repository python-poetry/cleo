...
class ListInputTest(CleoTestCase):
    ...
    def test_parse_arguments(self):
        input_ = ListInput([("name", "foo")], InputDefinition([InputArgument("name")]))
    ...
    def provide_options(self):
        return [
            ([("--foo", "bar")], [InputOption("foo")], {"foo": "bar"}),
            (
                [("--foo", "bar")],
            ...
            ([("-f", "bar")], [InputOption("foo", "f")], {"foo": "bar"}),
        ]

    def test_parse_invalid_input(self):
        for args, definition, expected in self.provider_invalid_input():
            self.assertRaisesRegexp(Exception, expected, ListInput, args, definition)
    ...
    def provider_invalid_input(self):
        return [
            ...
            ([("-f", "foo")], InputDefinition(), 'The "-f" option does not exist.'),
        ]
